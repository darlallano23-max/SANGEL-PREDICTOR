...
document.addEventListener("DOMContentLoaded", function() {
    console.log("Inicializando geovisor y asignando colores...");
    
    // 1. Crear el mapa apuntando al contenedor HTML
    map = L.map('map').setView([15, 10], 2);

    // 2. Capa base minimalista de CartoDB (hace que los colores resalten mucho más)
                ...
                .then(geojsonData => {
                    ...
                    geojsonLayer = L.geoJson(geojsonData, {
                        style: function(feature) {
                            // BUSCADOR ULTRA FLEXIBLE DE CÓDIGOS DE PAÍS (Prueba todas las variantes comunes)
                            const codigoGeo = (feature.id || feature.properties.id || feature.properties.iso_a2 || feature.properties.ISO_A2 || "").toUpperCase();
                            
                            // Buscar el país en tu lista de 17 países
                            const datosMatch = datosLogistica.find(p => p.pais_id.toUpperCase() === codigoGeo.trim());
                            
                            // Configurar colores según el Score de Riesgo Logístico
                            let colorFinal = '#e9ecef'; // Gris claro para países fuera del estudio
                            let opacidadFinal = 0.15;

                            if (datosMatch) {
                                const score = parseFloat(datosMatch.score_riesgo_logistico);
                                opacidadFinal = 0.75; // Resaltar nuestros 17 países
                            ...
                            return {
                                ...
                                opacity: 1,
                                color: datosMatch ? '#212529' : '#ced4da', // Bordes más marcados para tus países
                        ...
                        onEachFeature: function(feature, layer) {
                            ...
                            // Si es uno de tus países monitoreados, le activamos su ventana de información (Popup)
                            if (datosMatch) {
                                const contenidoPopup = `
                                    <div style="font-family: Arial, sans-serif; padding: 5px; min-width: 190px;">
                                        ...
                                        <table style="width: 100%; font-size: 12px; color: #495057;">
                                            ...
                                            <tr><td style="padding: 2px 0;">💱 <b>Volatilidad Divisa:</b></td><td style="text-align: right;">${datosMatch.volatilidad_divisa}/10</td></tr>
                                ...
                                // Efectos visuales interactivos al pasar el mouse (Hover)
                                layer.on({
                                    mouseover: function(e) {
                                        e.target.setStyle({ fillOpacity: 0.95, weight: 2.2, color: '#0d6efd' });
                                    },
                                    mouseout: function(e) {
